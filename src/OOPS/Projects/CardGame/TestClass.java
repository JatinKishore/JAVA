package OOPS.Projects.CardGame;
import java.util.*;
...
class Player {
    ...
    void addCard(String cardStr) {
        this.cards.add(new Card(cardStr));
    }
}
...
class Card implements Comparable<Card> {
    ...
    Card(String cardStr) {
        this.suit = cardStr.charAt(0);
        this.value = cardStr.substring(1);
    }
    ...
    public int compareTo(Card o) {
        ...
        return allValues.indexOf(this.value) - allValues.indexOf(o.value);
    }
}
...
class Game {
    ...
    Game(List<String> playerNames, char trump) {
        ...
        playerNames.forEach(name -> players.put(name, new Player(name)));
    }
    ...
    public Card getOptimalCard(Player self) {
        String teammate = (playerNames.indexOf(self.name) % 2 == 0)
                ? playerNames.get((playerNames.indexOf(self.name) + 2) % 4)
                : playerNames.get((playerNames.indexOf(self.name) + 2) % 4);

        Card highestCard = playedCards.get(0);
        for (Card card : playedCards) {
            // Determine the highest card played based on rules
        ...
        if (highestCard.suit != trump) {
            ...
            return (smallestBiggerCard != null) ? smallestBiggerCard : getSmallestCard(self.cards, '0');
        }
        ...
        if (highestTeammateTrumpCard == null || (highestEnemyTrumpCard != null && highestEnemyTrumpCard.compareTo(highestTeammateTrumpCard) > 0)) {
            ...
            return (smallestTrumpHigherThanEnemy != null) ? smallestTrumpHigherThanEnemy : getSmallestCard(self.cards, '0');
        }
    ...
    private Card getHighestCard(List<Card> cards, char suit) {
        // Find the highest card of a given suit in a list of cards
        return cards.stream()
    ...
    private Card getSmallestCard(List<Card> cards, char suit) {
        // Find the smallest card of a given suit in a list of cards
        return cards.stream()
                .filter(card -> suit == '0' || card.suit == suit)
                .min(Comparator.naturalOrder())
                .orElse(null);
    }

    private Card getSmallestCardBiggerThan(List<Card> cards, Card targetCard, char targetSuit) {
        // Find the smallest card bigger than a target card of a specific suit
        return cards.stream()
                .filter(card -> card.suit == targetSuit && card.compareTo(targetCard) > 0)
                .min(Comparator.naturalOrder())
                .orElse(null);
    }
}
...
public class TestClass {
    public static void main(String[] args) {
        ...
        // Print the optimal card to the console
